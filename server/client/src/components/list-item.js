...
const ItemList = ({el, endListReq,time,useWar,listReq}) => {
    ...
    const makeImportant = async () => {
        const res = await request("/api/main/change/important", "POST", {
            id: _id,
            important: important
    ...
    const makeDone = async () => {
        ...
        const res = await request("/api/main/change/done", "POST", {
            id: _id,
            done: done
        }, {Authorization: `Bearer ${auth.token}`})
        endListReq(!listReq)
    ...
    const deleteTodo = async () => {
        const res = await request("/api/main/change/delete", "POST", {id: _id}, {Authorization: `Bearer ${auth.token}`})
        endListReq(!listReq)
    }
    return (
