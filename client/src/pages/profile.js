...
const Profile = ({setPerson, setStatistic, person, edit, makeBtnReq , setWar,make_req }) => {
    ...
    const getProfileInfo = useCallback(async () => {
        try {
            setPerson({})
            const data = await request("https://tranquil-spire-85621.herokuapp.com/api/client/profile", "GET", null, {Authorization: `Bearer ${auth.token}`})
            setPerson(data)
            setWar(data.warning)
        }catch (e) {
        }
    ...
    const getStatisticInfo = async () => {
        const data = await request("https://tranquil-spire-85621.herokuapp.com/api/main/all", "GET", null, {Authorization: `Bearer ${auth.token}`})
        return data
    }

    const setCheck = async (val) =>{
        const res = await request("https://tranquil-spire-85621.herokuapp.com/api/client/warning","POST",{warning:val},{Authorization:`Bearer ${auth.token}`})
        makeBtnReq(!make_req)
        setWar(res.warning)
    }
