...
const AllTodos = ({todos, setTodos, skip, req, page, setTime, setWar}) => {
    ...
    const getTodosCount = async () => {
        const res = await request("https://tranquil-spire-85621.herokuapp.com/api/main/byPagination", "GET", null, {Authorization: `Bearer ${auth.token}`})
        return res
    }
    const getPaginationTodos = async (skip, limit) => {
        const res = await request("https://tranquil-spire-85621.herokuapp.com/api/main/byLimitSkip", "POST", {
            skip: skip,
            limit: limit
